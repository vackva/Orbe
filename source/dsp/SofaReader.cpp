...
void SofaReader::prepare(double samplerate) {
    ...
    int err;
    sofa = std::make_unique<MYSOFA_EASY*>(mysofa_open_data(BinaryData::pp1_HRIRs_measured_sofa, BinaryData::pp1_HRIRs_measured_sofaSize, static_cast<float>(samplerate), &ir_length, &err));
    switch (err) {
        case MYSOFA_OK:
