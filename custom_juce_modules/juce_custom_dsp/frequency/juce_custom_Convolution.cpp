...
struct CustomConvolutionMessageQueue::Impl  : public BackgroundMessageQueue
{
    using BackgroundMessageQueue::BackgroundMessageQueue;
};

CustomConvolutionMessageQueue::CustomConvolutionMessageQueue()
    : CustomConvolutionMessageQueue (1000)
{}

CustomConvolutionMessageQueue::CustomConvolutionMessageQueue (int entries)
...
CustomConvolutionMessageQueue::CustomConvolutionMessageQueue (CustomConvolutionMessageQueue&&) noexcept = default;
CustomConvolutionMessageQueue& CustomConvolutionMessageQueue::operator= (CustomConvolutionMessageQueue&&) noexcept = default;
...
public:
    ...
    void prepare (const ProcessSpec& spec)
    {
        smoother.reset (spec.sampleRate, 0.1);
...
using OptionalQueue = OptionalScopedPointer<CustomConvolutionMessageQueue>;
...
void CustomConvolution::Mixer::reset() { dryBlock.clear(); }

//==============================================================================
CustomConvolution::CustomConvolution()
    : CustomConvolution (Latency { 0 })
{}

CustomConvolution::CustomConvolution (CustomConvolutionMessageQueue& queue)
    : CustomConvolution (Latency { 0 }, queue)
{}

CustomConvolution::CustomConvolution (const Latency& requiredLatency)
    : CustomConvolution (requiredLatency,
                   {},
                   OptionalQueue { std::make_unique<CustomConvolutionMessageQueue>() })
{}

CustomConvolution::CustomConvolution (const NonUniform& nonUniform)
    : CustomConvolution ({},
                   nonUniform,
                   OptionalQueue { std::make_unique<CustomConvolutionMessageQueue>() })
{}

CustomConvolution::CustomConvolution (const Latency& requiredLatency, CustomConvolutionMessageQueue& queue)
    : CustomConvolution (requiredLatency, {}, OptionalQueue { queue })
{}

CustomConvolution::CustomConvolution (const NonUniform& nonUniform, CustomConvolutionMessageQueue& queue)
    : CustomConvolution ({}, nonUniform, OptionalQueue { queue })
{}

CustomConvolution::CustomConvolution (const Latency& latency,
                          const NonUniform& nonUniform,
                          OptionalQueue&& queue)
    : pimpl (std::make_unique<Impl> (latency, nonUniform, std::move (queue)))
{}

CustomConvolution::~CustomConvolution() noexcept = default;

void CustomConvolution::loadImpulseResponse (const void* sourceData,
                                       size_t sourceDataSize,
                                       Stereo stereo,
                                       Trim trim,
                                       size_t size,
...
void CustomConvolution::loadImpulseResponse (const File& fileImpulseResponse,
                                       Stereo stereo,
                                       Trim trim,
                                       size_t size,
                                       Normalise normalise)
...
void CustomConvolution::loadImpulseResponse (AudioBuffer<float>&& buffer,
                                       double originalSampleRate,
                                       Stereo stereo,
                                       Trim trim,
                                       Normalise normalise)
...
void CustomConvolution::prepare (const ProcessSpec& spec)
{
    mixer.prepare (spec);
    pimpl->prepare (spec);
    isActive = true;
...
void CustomConvolution::reset() noexcept
{
    mixer.reset();
    pimpl->reset();
}

void CustomConvolution::processSamples (const AudioBlock<const float>& input,
                                  AudioBlock<float>& output,
...
int CustomConvolution::getCurrentIRSize() const { return pimpl->getCurrentIRSize(); }

int CustomConvolution::getLatency() const { return pimpl->getLatency(); }
